  ...
  <div class="records-selection-list mt-3" style="display:none;width:auto !important;min-width:100% !important; max-width:100% !important;left:0px !important;" id="SelectionActivityOptions">
                      ...
                      <select class="form-select form-control" name="leads_assigned_by">
                          ...
                          <?php
                            if (AuthAppUser("UserType") == "ADMIN") {
                                $AllUsers = SET_SQL("SELECT UserId, UserFullName, UserPhoneNumber FROM users ORDER BY UserFullName", true);
                            } else {
                                $AllUsers = SET_SQL("SELECT UserId, UserFullName, UserPhoneNumber FROM users WHERE UserReportingManager='" . LOGIN_USER_ID . "' ORDER BY UserFullName", true);
                            }
                            if ($AllUsers != null) {
  ...
  <?php
    if (isset($_GET["ApplyAdvancefilters"])) {
        $GenerateSQL = "SELECT * FROM leads WHERE leads_is_removed!='1' and leads_managed_by='$SelectedUserId'";
        $filters = [
            "leads_type",
            "leads_stages",
            "leads_sub_stages",
  ...
  <div class="row">
      <div class="col-md-12">
          <?php echo PaginationFooter(TOTAL($PaginationTotalSQL), DOMAIN . "/mobile/teams"); ?>
      </div>
  </div>