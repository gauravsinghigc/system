<div class="row mb-4">
    ...
    if ($AllStages != null) {
        $LoginLevelConditions = " and UserReportingManager = '" . LOGIN_USER_ID . "' ";

        // Optimize: Fetch all counts in one query using GROUP BY
        $StageCountsQuery = "SELECT leads_stages, SUM(leads_type = 'LEAD') AS total_leads, SUM(leads_type = 'DATA') AS total_data, COUNT(*) AS total_records FROM leads, users WHERE leads.leads_managed_by=users.UserId $LoginLevelConditions GROUP BY leads_stages";
