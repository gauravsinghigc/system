...
if (isset($_POST['value']) && isset($_POST['name'])) {
    if ($_POST['name'] == "Tomorrow") {
        if (AuthAppUser("UserType") == "Admin") {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and DATE(LeadFollowUpDate)='" . date("Y-m-d", strtotime("+1 days")) . "' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
        } else {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads  where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and lead_followups.LeadFollowUpHandleBy='" . AuthAppUser("UserId") . "' and DATE(lead_followups.LeadFollowUpDate)='" . date("Y-m-d", strtotime("+1 days")) . "' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
        }
    } elseif ($_POST['name'] == "Missed") {
        if (AuthAppUser("UserType") == "Admin") {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and DATE(LeadFollowUpDate)<'$TDate' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
        } else {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads  where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and lead_followups.LeadFollowUpHandleBy='" . AuthAppUser("UserId") . "' and DATE(lead_followups.LeadFollowUpDate)<'$TDate' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
        }
    } elseif ($_POST['name'] == "customDate") {
        if (AuthAppUser("UserType") == "Admin") {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and DATE(LeadFollowUpDate)='$Missed' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
        } else {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads  where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and lead_followups.LeadFollowUpHandleBy='" . AuthAppUser("UserId") . "' and DATE(lead_followups.LeadFollowUpDate)='$Missed' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
        }
    } else {
        if (AuthAppUser("UserType") == "Admin") {
            $fetclFollowUps = _DB_COMMAND_("SELECT LeadFollowUpHandleBy, LeadFollowUpDescriptions, LeadFollowUpTime, LeadFollowCurrentStatus, LeadFollowMainId, LeadFollowUpUpdatedAt,LeadFollowStatus, LeadFollowUpDate FROM lead_followups, leads where leads.LeadsId=lead_followups.LeadFollowMainId and lead_followups.LeadFollowCurrentStatus like '%FOLLOW-UP%' and leads.CompanyID='$companyID' and DATE(LeadFollowUpDate)='$TDate' and lead_followups.LeadFollowUpRemindStatus='ACTIVE' ORDER BY lead_followups.LeadFollowUpId DESC", true);
...
?>
        <li>
            <span class='text-center bg-success text-black rounded'>
                <span class=''>
                    <?php if (DATE_FORMATES("h:i A", $F->LeadFollowUpUpdatedAt) == "NA") { ?>
                        <span class='h5'>No Call</span><br>
                    <?php } else { ?>
                        <span class="p-t-3 text-white">
                            <span class='h4 text-white'><i class='fa fa-clock'></i></span><br>
                            <span class='small'>scheduled for </span><br>
                            <span class='h5'> <?php echo $F->LeadFollowUpTime; ?></span><br>
